...
def generate_token() -> str:
    """Generate a local IDE Bridge API token."""
    return "ideb_" + secrets.token_urlsafe(32)


_TOKEN_RE = re.compile(r"ideb_[A-Za-z0-9_-]+")

# Fields whose values are redacted from logs to avoid leaking secrets.
_SECRET_RE = re.compile(
    r"(?i)\b(authorization|x-api-key|token|secret|password|api_key|api-key)\b['\"]?\s*[:=]\s*\S.*"
)
