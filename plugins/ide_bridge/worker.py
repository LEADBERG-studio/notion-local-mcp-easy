"""IDE Bridge worker: a stdlib HTTP server exposing the full OpenAI-compatible
...
from plugins.ide_bridge.openai_schemas import (
...
from plugins.ide_bridge.openai_translate import (
...
from plugins.ide_bridge.openai_fallbacks import (
    embeddings_response,
    normalize_inputs,
    moderations_response,
)
from plugins.ide_bridge.openai_media import (
    PUBLISH_HINT,
    extract_urls,
    extract_artifacts,
    first_url,
...
from plugins.ide_bridge.openai_toolbridge import (
    catalog,
...
from plugins.ide_bridge.openai_streaming import (
...
from plugins.ide_bridge.security import check_auth, redact_line
...
class _NoReuseHTTPServer(ThreadingHTTPServer):
    ...
    # ide_bridge endpoint a duplicate bind must fail loudly.
...
def _openai_error(message: str, code: str, type_: str = "ide_bridge_error",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _models_response(self) -> dict[str, Any]:
        model_id = STATE.get("model_id", "ide-bridge")
        gw_mode = str(STATE.get("gateway_mode", "bridge")).lower()
        if gw_mode == "external":
            from plugins.ide_bridge.backend import discover_models
            cfg = {
    ...
    def _tool_call_completion(self, model: str, tool_name: str, arguments: dict) -> dict[str, Any]:
        return {
            ...
            "system_fingerprint": "ide-bridge-toolrunner",
            "choices": [{
                "index": 0,
    ...
    def _handle_images_generations(self) -> None:
        ...
        request_data = {
            ...
            "model": validated["model"] or STATE.get("model_id", "ide-bridge"),
    ...
    def _handle_images_edits(self) -> None:
        ...
        request_data = {
            ...
            "model": model or STATE.get("model_id", "ide-bridge"),
    ...
    def _handle_audio_speech(self) -> None:
        ...
        try:
            ...
            self._send_json(200, speech_url_response(url, validated["model"] or STATE.get("model_id", "ide-bridge")))
    ...
    def _handle_audio_transcription(self, translate: bool) -> None:
        ...
        request_data = {
            ...
            "model": STATE.get("model_id", "ide-bridge"),
