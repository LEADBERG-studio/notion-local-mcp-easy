...
from plugins.ide_provider.queue import clean_queue, request_counts
from plugins.ide_provider.security import generate_token, redact_line


DEFAULT_CONFIG = {
    "default_host": "127.0.0.1",
    "port_range": [8787, 8899],
    "default_model_id": "ide-provider",
...
def runtime_root(context: dict[str, Any]) -> Path:
    ...
    return workspace / "temp" / "ide_provider_runtime"
...
def safe_runtime_path(context: dict[str, Any], *parts: str) -> Path:
    ...
    if root != path and root not in path.parents:
        raise ValueError("runtime path escapes ide_provider runtime root")
    return path
...
# Strong references to started worker Popen objects so their __del__ does not
# fire ResourceWarning while the process is alive; _kill_pid reaps them.
...
def _reap_proc(pid: int, timeout: float = 5.0) -> None:
    ...
    if proc.poll() is None:
        try:
            ...
            pass  # already signalled by _kill_pid; nothing more to do
...
def _kill_pid(pid: int) -> None:
    ...
    # Reap the child so it does not become a zombie (POSIX) and so the Popen
    # file descriptors are closed before the strong reference is dropped.
    _reap_proc(pid)
...
def start_endpoint(arguments: dict[str, Any], context: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    # Idempotent same-endpoint check MUST happen before port-free check: a
    # running worker legitimately occupies its port, so is_port_free would
    # otherwise (wrongly) reject a restart of the same endpoint name.
    ...
    for sub in ("endpoints", "queues", "logs", "pids"):
    ...
    # Keep a strong reference so Popen.__del__ does not fire a ResourceWarning
    # while the worker is still alive; _kill_pid/stop_endpoint reap it.
    ...
    if not ready:
        ...
        return {
            "ok": False,
            "status": "failed",
            "message": f"worker did not become healthy: {last_err}",
        }
    ...
    return {
        ...
        "message": "Endpoint is running. Add it as an OpenAI-compatible provider in your IDE. Keep this chat active and call ide_provider_wait_request to serve requests.",
    }
