...
Новые версии сохраняют настройки каждого режима отдельно в `%LOCALAPPDATA%\NotionMcpEasy\connection-profiles.json`, а активный набор зеркалят в legacy `config.json` для старых версий. При старте Enter оставляет текущий режим; смена режима предлагает использовать сохранённый профиль или запустить явный setup. Токены и tunnel backend при обновлении не перевыпускаются и не угадываются. Перед записью config создаётся backup, сохраняются последние 5 backup/log-файлов.
