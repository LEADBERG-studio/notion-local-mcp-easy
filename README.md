...
4. Настройте профиль подключения через `PROFILES.bat`, затем выберите его в `SETUP.bat`. Доступны: **Serveo stable**, **Serveo temporary**, **Tunnellio stable**, **Tunnellio random**, **Tunnellio direct TCP bridge**, **Self-hosted sish relay** и **Custom public URL / reverse proxy**. Быстрее всего стартовать с **Tunnellio direct TCP bridge** или **Serveo temporary**: они не требуют ввода вообще.
...
## Инструменты для экономных правок

Четыре инструмента, которые заметно сокращают и число запросов, и расход токенов:

| Инструмент | Зачем |
| --- | --- |
| `read_many_files` | Прочитать список файлов одним запросом. Серия мелких запросов через туннель это самая опасная нагрузка. |
| `apply_patch` | Unified diff вместо перезаписи файла. Diff стоит только изменения, перезапись стоит двух объёмов файла. |
| `search_and_replace` | Замена по проекту. По умолчанию только показывает, что изменится, с примером строки по каждому файлу. |
| `tail_file` | Конец текстового файла. Интересное в логе почти всегда в конце. |

`apply_patch` применяет либо все ханки, либо ни один, поэтому файл никогда не остаётся в состоянии, которого никто не описывал. Номера строк считаются подсказкой: контекст ищется рядом, так что слегка устаревший diff всё равно применяется.

## Стабильность транспорта

Сервер почти всегда доступен через туннель, а туннель это один TCP-путь с прокси на каждом конце. Что сделано, чтобы запросы не срывались:

- **Соединение держится открытым.** По умолчанию у веб-сервера keep-alive 5 секунд, и клиент успевал отправить запрос в момент закрытия сокета: relay отвечал `502 Bad Gateway`. Теперь 120 секунд.
- **Ответы сжимаются** начиная с 1 КБ. JSON это текст, он ужимается примерно на порядок.
- **Повторы распознаются.** Переотправленный запрос воспроизводится из короткого окна, а не выполняется дважды.
- **Повторные чтения кэшируются** на несколько секунд. Любая операция изменения сбрасывает этот кэш, поэтому устаревших данных после записи не бывает.
- **Приём ограничен.** Сверх лимита сервер честно отказывает с `Retry-After`, а не складывает работу туда, где её не видно.
- **Вывод ограничен** у встроенных инструментов, у плагинов и у баз данных.

Проверить состояние: инструмент `transport_health`. Он отличает настоящую ошибку от сброса нагрузки. Найти висящие процессы туннелей: `DOCTOR.bat`.
...
### Профили подключения (2.4.0)

В 2.4.0 настройки подключения полностью переработаны. Раньше все режимы жили в одном плоском конфиге, поля пересекались, а недостающие значения «восстанавливались» из случайного backup. Из-за этого проблема в одном канале валила все остальные. Теперь у каждого вида подключения свой независимый контур.

**Три отдельных шага вместо одного мастера:**

| Скрипт | Что делает | Что спрашивает |
| --- | --- | --- |
| `PROFILES.bat` | Создаёт и правит профили подключения | Только то, что нужно конкретному протоколу |
| `SETUP.bat` | Настраивает рабочую область | Папка, режим доступа, готовый профиль |
| `START.bat` | Запускает сервер | Только выбор рабочей области. Если у папки уже есть профиль, вопросов нет вовсе |

**Профиль подключения это именованный экземпляр протокола.** Один и тот же
протокол можно сохранить несколько раз с разными доменами и ключами: `Prod MCP`
и `Staging MCP` оба могут быть Tunnellio stable, но с разными резервациями.
Все сохранённые профили лежат в одном пронумерованном списке, выбор `1-N`
подключает в одно движение. В списке всегда видны сами настройки, а не только
имя: без этого нельзя решить, какой профиль нужен.

**Семь протоколов, на которых строятся профили:**

1. **Serveo stable domain** — вы сами генерируете SSH-ключ и резервируете имя в аккаунте Serveo. Скрипт спрашивает только ключ и зарезервированное имя.
2. **Serveo temporary domain** — не спрашивает ничего. Домен случайный и меняется после переподключений.
3. **Tunnellio stable domain** — вы сами генерируете SSH-ключ и резервируете домен в кабинете Tunnellio. Скрипт спрашивает только ключ и домен. Больше ничего для этого режима не требуется.
4. **Tunnellio random domain** — только API-ключ. Скрипт сразу проверяет его на сервере Tunnellio и сохраняет профиль только после подтверждения.
5. **Tunnellio direct TCP bridge** — keyless нативный мост. Можно не заполнять вообще ничего: сервер выдаст случайный домен. Зарезервированный домен и API-ключ опциональны.
6. **Self-hosted sish relay** — ваш собственный relay: хост, порт, wildcard-домен, subdomain и ключ.
7. **Custom public URL / reverse proxy** — только публичный origin, встроенный туннель не поднимается.

**Как это устроено внутри:**

- У каждого профиля есть неизменяемый прообраз в `connections/defaults/<id>.json`. Он едет в релизе, руками не правится и содержит все значения по умолчанию. При первой настройке профиль клонируется из прообраза, поэтому вам не приходится вводить служебные параметры.
- Настроенные профили лежат в `%LOCALAPPDATA%\NotionMcpEasy\connection-profiles.v2.json`. **Обновление продукта их не трогает.** Они меняются только когда вы сами запустили `PROFILES.bat`.
- Рабочие области и активный выбор лежат в одном постоянном файле `%LOCALAPPDATA%\NotionMcpEasy\current-connection.json`.
- Каждому виду подключения соответствует свой модуль кода в `connections/circuits/`. Контур не видит поля чужих контуров: он физически не может потребовать ключ от другого режима или преобразовать домен по чужим правилам.
- Никаких молчаливых переключений между каналами. Если активный профиль не поднялся, launcher скажет об этом прямо. Сменить канал можно только явно через `SETUP.bat`.
- Legacy `config.json` продолжает создаваться для обратной совместимости, но теперь это **генерируемое зеркало** активного профиля, а не источник истины. Поля неактивных контуров в нём всегда пустые.
- Вместо растущей кучи `config.backup.*.json` остался один rolling `config.json.bak`.

**Токены.** По умолчанию у каждой рабочей папки свой MCP-токен и свой OAuth owner code, так что разные проекты изолированы. В `SETUP.bat` есть отдельный вопрос: использовать общие учётные данные для всех областей. Включите его, если хотите переключать каналы подключения, не переподключая MCP-клиентов заново.
...
- `START.bat` — создать локальное `.venv`, установить зависимости и запустить сервер с туннелем. Если сохранённых рабочих областей несколько, launcher покажет их список. Больше ничего при старте не спрашивается: активный профиль подключения поднимается как есть, без угадываний.



- `PROFILES.bat` — настроить профили подключения. Показывает все семь профилей со статусом, позволяет настроить, проверить подключение, посмотреть неизменяемый прообраз и сбросить профиль к нему.



- `DOCTOR.bat` — показать все запущенные процессы туннелей и пометить тот, которым владеет текущий launcher. Всё, что помечено `orphan`, осталось от прошлого запуска и может держать порт на relay. `DOCTOR.bat --cleanup` предложит их остановить по одному; живой туннель никогда не трогается.
...
- `SETUP.bat` — настроить рабочую область: папка, режим доступа и готовый профиль подключения. Токены при повторном setup сохраняются.
